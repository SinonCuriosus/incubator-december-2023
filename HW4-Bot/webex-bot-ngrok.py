...
import requests
import os
from footballFanHelper import FootballFixture
...
my_bot_token = "ODZiYmE5NjktMjk5MC00M2IxLTkwNjEtOGYwYWFiMjE2NmY4NTk0ZWFiNTMtMjI4_P0A1_db9ced2e-7d22-4ed1-9efc-08b3b5cb317c"
...
def process_message(message_obj):
    ...
    if "hello" in message_obj.text.lower():
        ...
        print(message_obj)
        msg_result = api.messages.create(toPersonEmail=message_obj.personEmail, markdown="Hello to you to! Give me a club name that you are fan or you have bet on.")
        return msg_result
    else:
        ...
        team_id = fixture.get_team_id(message_obj.text)

        if team_id:
            fixture.querystring["team"] = team_id
            fixture.fetch_data()
            msg_result = api.messages.create(toPersonEmail=message_obj.personEmail, markdown="" + fixture.print_match_info()+ "\nCuriosity in any other club?")
            return msg_result
    msg_result = api.messages.create(toPersonEmail=message_obj.personEmail, markdown="" + "I wasn't able to get a club name from that.\nMay you specify the club name better?")
    return msg_result
